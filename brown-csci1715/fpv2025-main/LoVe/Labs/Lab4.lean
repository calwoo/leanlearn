...
Our intention is that `reverseAccu [] xs` should be equal to
...
Implement the function `dotProduct` that computes the dot product of two
...
We define the *melding* (a coined term) of two equal-length vectors to
...
The *zipping* of two vectors is the vector formed by pairing
...
Prove the lemma below that says that appending and melding can be done
...
Having defined vectors, we now turn our attention to matrices. An
...
The first kind of matrix operation we'll implement is extracting
...
Our second matrix operation is addition. Matrix addition proceeds
component-wise: e.g.,
┌     ┐   ┌     ┐     ┌         ┐
| a b | + | e f |  =  | a+e b+f |
| c d |   | g h |     | c+g d+h |
...
Define the `take` function, which returns a list consisting of the the
...
Prove the following theorems, using `induction` or pattern matching.
...
Follow the recursion pattern of `drop` and `take` to prove the
...
Define an inductive type corresponding to the terms of the untyped
...
Register a textual representation of the type `term` as an instance of
...
Test your textual representation. The following command should print
